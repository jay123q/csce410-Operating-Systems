...
void pass_on_CPU(Thread *_to_thread, int counter)
{


// I cannot explain why, but modifying this functon fixed a error with mirror test where it would run through, complete, reboot, and fail
// help
    ...
    Console::puts(" do you drop the thread \n");
    SYSTEM_SCHEDULER->resume(Thread::CurrentThread());
    Console::puts(" do you did good boy! up \n");
    SYSTEM_SCHEDULER->yield();
    Console::puts(" do now role over \n");
       Machine::enable_interrupts();
    #ifdef MIRROR_TEST
    if( counter == 4)
    {
        Console::puts(" stopping output here otherwise very hard to read \n ");
        assert(false);
    }
    #endif
#endif
...
void fun4()
{
    ...
    for (int j = 0;; j++)
    {
        ...
        pass_on_CPU(thread1,1);
    }
    #ifdef MIRROR_TEST
     Console::puts(" stopping output here otherwise it runs too fast to see HALT KERNEL 'n");
    #endif
}
