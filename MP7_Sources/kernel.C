...
void exercise_file_system(FileSystem * _file_system, int stopCounter) {
    ...
    // {
    //     /* code */
    //     Console::puti(_file_system->inodeList[i]->fileId);
    // }
    ...
    
    {
        ...
        // Console::puts(" write file 1 check \n ");
        //  char  buffer[20];
        // file2.Read(20,buffer);
        // Console::puts(buffer);
        /* -- Files will get automatically closed when we leave scope  -- */
        Console::puts(" done writing and creating files ! \n ");
    }
    ...
    {   
        ...
        File file1(_file_system, 1);
        File file2(_file_system, 2);
        // Console::puts(" file 1 blokc number should be where diskk is stored "); Console::puti(file1.fileId);
        ...
        for(int i = 0; i < 20; i++) {
            Console::puts(" IABFJKSNF IPSJBKDNAK K,H CKBHJHJVCLH,ASVUKA \n");
            Console::puts("result1: "); Console::puts((const char *) result1[i]); Console::puts(" |string 1 is: "); Console::puts( (const char *) STRING1[i]);Console::puts(" dead space \n");
             assert(result1[i] == STRING1[i]);
        }
        Console::puts(" opening file 1, and reading \n ");
        ...
        Console::puts(" opening file 2, and reading \n ");
        /* -- "Close" files again -- */
    }
    /* -- Delete both files -- */
    ...
    if( stopCounter > 20 )
    {
        Console::puts(" 20 ITERATIONS HIT STOPPING");
        Console::puts(" 20 ITERATIONS HIT STOPPING");
        Console::puts(" 20 ITERATIONS HIT STOPPING");
        Console::puts(" 20 ITERATIONS HIT STOPPING");
        Console::puts(" 20 ITERATIONS HIT STOPPING");
