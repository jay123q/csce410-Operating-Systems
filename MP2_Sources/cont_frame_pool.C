...
ContFramePool::FrameState ContFramePool::get_state(unsigned long _frame_no)
{
    ...
    if ( (bitmap[bitmap_index] & hosMask) == 1 )
    {
                             ...
                             unsigned long _info_frame_no)
{
    ...
    assert((n_frames % 8) == 0) for (int i = 0; i < _n_frames; i++)
    {
...
unsigned long ContFramePool::get_frames(unsigned int _n_frames)
{
    ...
    while (index + n_free_frames < n_frames)
    {
        Console::puts("a");
        // Console::puts("\n");
        // Console::puti(get_state(index) == FrameState::Free);
        // Console::puts("\n");
        if (get_state(index) == FrameState::Free)
        {
            Console::puts("free frame index ");
            bool segAvail = true;
                // Console::puts("\n n = index+1 \n ");
                // Console::puti(index + 1);
                // Console::puts("\n index+needed_frames \n");
                // Console::puti(index + needed_frames);
            for (int n = index + 1; (n < index + needed_frames) && segAvail ; n++)
            {   
                // Console::puts("\n ");
                // Console::puti(n);
            ...
            if (segAvail == true)
            {
                ...
                for (int frameNumber = index + 1; frameNumber < index + needed_frames; frameNumber++)
                {
    ...
    Console::puts("\n hey bud, i dont know how you got here but it is not good Cont_frame_pool.C, 269 \n");
    assert(false)
    // TODO: IMPLEMENTATION NEEEDED!
}
                                      ...
                                      unsigned long _n_frames)
{
    ...
    for (unsigned long index = _base_frame_no; index < _base_frame_no + _n_frames; index++)
    {
        set_state(index - this->base_frame_no, FrameState::Used);
    }
}
void ContFramePool::single_release(unsigned long _first_frame_no)
{
    unsigned long fno = _first_frame_no - this->base_frame_no;
    assert(get_state(fno) == FrameState::HoS);
...
bool ContFramePool::contains(unsigned long _frame_check)
{
    return (_frame_check >= this->base_frame_no && _frame_check <= this->base_frame_no + this->n_frames);
}
void ContFramePool::release_frames(unsigned long _first_frame_no)
{
    // TODO: IMPLEMENTATION NEEEDED!
    for (ContFramePool *framePool = head_frame; framePool != nullptr; framePool = framePool->next_frame)
    {
        if (framePool->contains(_first_frame_no))
        {
            framePool->single_release(_first_frame_no);
            return;
        }
    }
}
