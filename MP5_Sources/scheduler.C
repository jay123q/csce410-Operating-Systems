...
void Scheduler::resume(Thread *_thread)
{
  ...
  linked_queue * new_thread = new linked_queue { _thread, nullptr };
...
RoundRobin::RoundRobin( int timer )
{
  // kill  the thread then thread = thread->next
    linked_queue * RRhead = nullptr;
    linked_queue * RRtail = nullptr;
    quantum = timer;
